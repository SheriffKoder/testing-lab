...
**`npm run build`**. Playwright is a **fourth** job in Item 5
...
Item 4 leaves coverage upload off. Playwright reports upload on e2e
failure (Item 5). Do not upload secrets. Do not treat `.next` as a
deploy pipeline yet (Item 8).
...
`npm run verify` exists in `package.json` (full PR sequence). Do not
hook it to the workflow.
...
| Topology | **B** then Item 5 — four jobs: `quality`, `tests`, `build`, `e2e` |
| `needs` | **None** — all four start together |
| Node | `24.12.0` (`.nvmrc` + `setup-node` on every job) |
| Cache | `cache: npm` on every job (lockfile key) |
| Gates | lint + typecheck / `npm test` / `npm run build` / `npm run test:e2e` |
| Local replica | `npm run ci` → lint + typecheck + test + test:e2e + build (serial) |
| Artifacts | Playwright report + `test-results/` on **e2e failure** (Item 5) |
...
parallelize four runners; it runs the same five commands in order. It
is **not** `npm ci` (the lockfile install).

`npm run verify` is the same five commands. It must **not** be the
workflow command.

`cache: npm` speeds download. Correctness still comes from `npm ci`.
A cache is for later runs. An artifact keeps this run’s output — the
e2e job uploads the Playwright report on failure
([`3-5-playwright-in-ci.md`](./3-5-playwright-in-ci.md)).

`npm run build` is a PR and `main` gate. Playwright is the `e2e` job.
...
`build`, or `e2e`. You can see more than one layer fail in the same run.
...
Same five commands as the YAML, serial on your machine. CI still
